...
public final class TwitterColumnFactory {
	...
	public static Column homeTimeline (final int id, final Account account) {
		...
		return new Column(id, "Home Timeline", account.getId(), MainFeeds.TIMELINE.name(), DEFAULT_REFRESH_MINS, null, null, false);
	}

	public static Column mentions (final int id, final Account account) {
		checkAccount(account);
		return new Column(id, "Mentions", account.getId(), MainFeeds.MENTIONS.name(), DEFAULT_REFRESH_MINS, null, null, false);
	}
