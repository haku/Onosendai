...
//	Activity.
	...
	public void storeTweets (int columnId, List<Tweet> tweets) {
		...
		try {
			for (Tweet tweet : tweets) {
				...
				long ret = this.mDb.insert(TBL_TW, null, values);
	...
	@Override
	public List<Tweet> getTweets (int columnId, int numberOf) {
		if (this.mDb == null) {
		...
		try {
			c = this.mDb.query(true, TBL_TW,
					new String[] { TBL_TW_NAME, TBL_TW_BODY, TBL_TW_TIME },
			...
			if (c != null && c.moveToFirst()) {
				...
				do {
					...
					ret.add(new Tweet(name, body, time));
				}
				while (c.moveToNext());
			}
		}
